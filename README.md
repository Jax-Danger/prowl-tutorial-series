...
**Video:** coming. There is no public Part 4 URL yet. The steps below are the player that is in this branch.

Part 4. After the loading screen you land in the Game scene and move. **WASD** walks. **Space** jumps. The player in this project is `Assets/Scripts/Player.cs` on the **Player** prefab, with a **CharacterController** and a **Floor** under it.

Engine download: https://github.com/ProwlEngine/Prowl (this series uses **1.0-preview-4**).

This clone is the Prowl project root. `Assets/Scenes`, `Assets/Prefabs`, and `Assets/Scripts` are the title, loading, and game work from the earlier parts, plus the player.
